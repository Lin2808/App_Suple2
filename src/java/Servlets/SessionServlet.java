    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (opcion) {
            case "log":
                ...
                UsuarioController conemplo = new UsuarioController();
                usr = conemplo.login(usuario, clave);
                if (usr != null) {
                    HttpSession session = request.getSession();
                    session.setAttribute("usuario", usr);
                    if (usr.getEstado().equals("a")) {
                        if (usr.getCargo().equals("tienda")) {
                            response_server = "{\"redirect\":\"tienda.html\"}";
                        } else if (usr.getCargo().equals("cliente")) {
            ...
            case "ses":
                ...
                if (usr != null) {
                    String nombre = String.format("%s %s", usr.getNombre().trim(), 
                            usr.getApellido().trim());
                    String infoUser = String.format("{\"getNombres\":\"%s\","
                            + "\"getNombreUsuario\":\"%s\",\"getCargo\":\"%s\"}", 
                            nombre, usr.getNombre_usuario(), usr.getCargo());
                    if (usr.getCargo().equals("tienda")) {
                        response_server = "{\"redirect\":\"tienda.html\","
                                + "\"userObject\":" + infoUser + "}";
                    } else if (usr.getCargo().equals("cliente")) {
            ...
            default: //cerrar sesion
                ...
                response_server = 
                        "{\"redirect\":\"index.html\",\"permiso\":\"nothing\"}";
                break;
        }
        processRequest(request, response);
    }
