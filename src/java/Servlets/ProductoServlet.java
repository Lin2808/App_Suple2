...
public class ProductoServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        ProductoController pcon = new ProductoController();
        switch (this.opcion) {
            case "listarProductos":
                usr = (UserSession) request.getSession().getAttribute("usuario");
                response_server = pcon.listarProductos(usr.getId_user());
                break;
            case "listarProductosTienda":
                String id_usuario = request.getParameter("id_usuario");
                response_server = pcon.listarProductosTienda(id_usuario);
                break;
            case "insertarProducto":
                ...
                if (usr != null) {
                    this.response_server = "{\"message\":\"" + 
                            pcon.insertarProducto(nombre, stock, precio, 
                                    usr.getId_user()) + "\"}";
                } else {
